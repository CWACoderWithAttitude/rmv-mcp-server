...
class RMVService:
    ...
    def __init__(self, api_key: str = ""):
        ...
        self.api_key = api_key or os.getenv("RMV_API_KEY", "")
    ...
    async def get_connections(
        ...
        return str({
            "origin": result.get("Trip", [{}])[0].get("LegList", {}).get("Leg", [{}])[0].get("Origin", {}).get("name"),
            "destination": result.get("Trip", [{}])[0].get("LegList", {}).get("Leg", [{}])[-1].get("Destination", {}).get("name") if result.get("Trip") else None,
            "trips": trips,
            "count": len(trips)
        })